...
#if defined(NDEBUG)
# error "Quantiumnote cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xbf02324a292c770ada1b9efe2d5689bd8116d002e59fee5dd562311b208d07e5");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Quantiumnote: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Quantiumnote Signed Message:\n";
...
{
    // Quantiumnote: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 1000); // Quantiumnote: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Quantiumnote: 3.5 days
static const int64 nTargetSpacing = 3 * 60; // Quantiumnote: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Quantiumnote: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Quantiumnote: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x217b9f6b285c4c9ee10f5f23991c778ede975c2be2d6222258a2d904e71b4643");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "QuantiumCoin_0.1";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50000 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("046cf513460577be0ac6afc17b2d8d3d01d176a2a945f05419246020319495db47944d0a12874ed0dfd299ebe0972a141e5b36ef54aebaa6e1e67b4d833ed5f92e") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1520057488;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2087032066;

        if (fTestNet)
        {
            block.nTime    = 1520057474;
            block.nNonce   = 386252776;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x7cd34e8a24db98b3cc664ef4b5c51e068b3125f4c4b004dd620e7d893fe93b42"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfc, 0xc1, 0xba, 0xd3 }; // Quantiumnote: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("QuantiumnoteMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("QuantiumnoteMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static QuantiumnoteMiner(CWallet *pwallet)
{
    printf("QuantiumnoteMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("quantiumnote-miner");
    ...
    try { loop {
        ...
        printf("Running QuantiumnoteMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("QuantiumnoteMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&QuantiumnoteMiner, pwallet));
}
